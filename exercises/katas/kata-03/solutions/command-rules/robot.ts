...
/**
 * Whether a command is even legal from the robot's current position and
 * cargo - pulled out of `runCommand` and `replayBatch`'s forward loop: it's
 * the rule most likely to grow a new business exception next.
 */
function validateCommand(state: RobotState, command: RobotCommand): void {
 ...
 * button - one level of history, not a batch rollback. Inverses are always
 * legal by construction, so this skips `validateCommand`.
 */
