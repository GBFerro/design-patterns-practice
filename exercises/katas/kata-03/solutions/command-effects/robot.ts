...
/**
 * What a command does to the robot's state - move, pick up, drop off -
 * pulled out of every place that applies a command, forward or inverted:
 * `runCommand`, both loops in `replayBatch`, and `undoLast`.
 */
function applyEffect(state: RobotState, command: RobotCommand): RobotState {
...
export function runCommand(state: RobotState, command: RobotCommand): RobotState {
  ...
  return applyEffect(state, command);
}
...
export function replayBatch(
  ...
  for (let i = 0; i < commands.length; i++) {
    ...
    try {
      ...
      current = applyEffect(current, command);
    } catch (err) {
      ...
      for (let j = journal.length - 1; j >= 0; j--) {
        ...
        rollback = applyEffect(rollback, inverse);
      }
...
export function undoLast(
  ...
  return applyEffect(currentState, inverse);
}
