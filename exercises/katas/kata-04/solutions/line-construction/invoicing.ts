import type { Invoice, InvoiceLine, Order, OrderLine } from "./types.ts";

const LARGE_ORDER_THRESHOLD_CENTS = 100_000;

/**
 * What one order line becomes on the invoice - pulled out of both
 * `finalizeInvoice` and `finalizeInvoiceBatch`: it's the rule most likely to
 * gain a new line kind, or a new rule for an existing one.
 */
function buildInvoiceLine(line: OrderLine): InvoiceLine {
...
export function finalizeInvoice(order: Order): Invoice {
  const lines: InvoiceLine[] = order.lines.map(buildInvoiceLine);
