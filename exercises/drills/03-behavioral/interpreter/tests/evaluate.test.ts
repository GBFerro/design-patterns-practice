...
test("two comparisons joined by && both have to pass", () => {
  assert.equal(
    evaluateConstraint("altitude > 30 && moon_phase < 0.3", baseContext),
    true,
  );
  assert.equal(
    evaluateConstraint("altitude > 30 && moon_phase < 0.1", baseContext),
    false,
  );
});
...
test("three clauses joined by && all have to pass", () => {
  ...
  assert.equal(
    evaluateConstraint(expression, { ...baseContext, seeingArcsec: 3.0 }),
    false,
  );
});
...
test("civil_twilight_over reads the matching flag off the context", () => {
  ...
  assert.equal(
    evaluateConstraint("civil_twilight_over", {
      ...baseContext,
      civilTwilightOver: false,
    }),
    false,
  );
});
...
test("dome_open reads the matching flag off the context", () => {
  assert.equal(
    evaluateConstraint("dome_open", { ...baseContext, domeOpen: false }),
    false,
  );
});
...
test("a flag and a comparison combine with &&", () => {
  ...
  assert.equal(evaluateConstraint(expression, baseContext), true);
  assert.equal(
    evaluateConstraint(expression, { ...baseContext, domeOpen: false }),
    false,
  );
});
...
test("surrounding whitespace around clauses and operators is tolerated", () => {
  assert.equal(
    evaluateConstraint("  altitude   >   30   &&   clear  ", baseContext),
    true,
  );
});
