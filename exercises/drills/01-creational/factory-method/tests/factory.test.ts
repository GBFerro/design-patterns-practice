...
import { createJobFromBatchImport, createJobFromCounter, createJobFromReprint, type OrderRequest } from "#exercise";
...
for (const [siteName, createJob] of Object.entries(CALL_SITES)) {
  test(`${siteName}: business cards describe themselves and estimate their own time`, () => {
    const job = createJob({ jobKind: "business-cards", customerName: "Rowan Ito", quantity: 250 });
  ...
  test(`${siteName}: double-sided business cards cost more time and say so`, () => {
    const job = createJob({ jobKind: "business-cards", customerName: "Rowan Ito", quantity: 250, doubleSided: true });
  ...
  test(`${siteName}: a brochure defaults to bi-fold`, () => {
    const job = createJob({ jobKind: "brochure", customerName: "Priya Nair", quantity: 40 });
    assert.equal(job.describe(), "40 bi-fold brochures for Priya Nair");
    assert.equal(job.estimatedMinutes(), 8);
  });

  test(`${siteName}: a tri-fold brochure costs more time`, () => {
    const job = createJob({ jobKind: "brochure", customerName: "Priya Nair", quantity: 40, foldType: "tri" });
  ...
  test(`${siteName}: a banner defaults to 90cm wide`, () => {
    const job = createJob({ jobKind: "banner", customerName: "Delacroix Signs", quantity: 2 });
  ...
  test(`${siteName}: a wider banner costs more time`, () => {
    const job = createJob({ jobKind: "banner", customerName: "Delacroix Signs", quantity: 2, widthCm: 180 });
    assert.equal(job.estimatedMinutes(), 21);
  });
}

test("the three call sites agree on every field for the same order", () => {
  const request: OrderRequest = { jobKind: "brochure", customerName: "Priya Nair", quantity: 40, foldType: "tri" };
  const jobs = [createJobFromCounter(request), createJobFromReprint(request), createJobFromBatchImport(request)];
