...
import { createJobFromBatchImport, createJobFromCounter, createJobFromReprint } from "#exercise";
...
for (const [siteName, createJob] of Object.entries(CALL_SITES)) {
  test(`${siteName}: foil stamping defaults to gold foil`, () => {
    const job = createJob({ jobKind: "foil-stamping", customerName: "Odell & Vance", quantity: 30 });
  ...
  test(`${siteName}: a named foil color is honored`, () => {
    const job = createJob({ jobKind: "foil-stamping", customerName: "Odell & Vance", quantity: 30, foilColor: "silver" });
    assert.equal(job.describe(), "30 foil-stamped pieces for Odell & Vance (silver foil)");
  });

  test(`${siteName}: the time estimate grows with quantity`, () => {
    const job = createJob({ jobKind: "foil-stamping", customerName: "Odell & Vance", quantity: 120 });
